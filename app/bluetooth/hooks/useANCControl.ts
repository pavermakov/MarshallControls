...
const useANCControl = (device: Device | null) => {
    ...
    useEffect(() => {
        ...
        const subscription = device.monitorCharacteristicForService(MARSHALL_SERVICE_UUID, MARSHALL_ANC_CHAR_UUID, (error, characteristic) => {
    ...
    const setANCMode = useCallback(async (mode: ANCMode): Promise<void> => {
        if (!device) {
            return;
        }
        ...
        try {
            ...
            await device.writeCharacteristicWithResponseForService(
                ...
                MARSHALL_ANC_CHAR_UUID, encoded
            );

            setCurrentMode(mode);

        } catch(error: any) {
            setError('Failed to set ANC mode. Are your headphones connected?');
        } finally {
            setIsLoading(false);
        }
    }, [device]);
